...
acceleration_history = np.zeros((n, 4, 1))  # acceleration history
...
for i in range(n):
    ...
    derivative = A @ np.array([[state[4, 0]], [state[5, 0]], [state[0, 0]], [state[1, 0]]]) + B @ np.array([[road_profile[0, 0]], [u[0, 0]]])

    # calculate the acceleration
    acceleration = C @ np.array([[state[4, 0]], [state[5, 0]], [state[0, 0]], [state[1, 0]]])

    # update the state
    change = dt * derivative
    state = state + np.array([[change[2, 0]], [change[3, 0]], [0], [0], [change[0, 0]], [change[1, 0]], [0], [0]])
